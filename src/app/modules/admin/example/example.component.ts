...
export class ExampleComponent implements OnInit {
    ...
    pageForm = this.fb.group({
        ...
        viewId: [null as number | null, Validators.required],
    ...
    savePage(): void {
        ...
        const payload: CreatePagePayload = {
            ...
            viewId: this.pageForm.value.viewId!,
    ...
    pageFieldOptions(): string[] {
        const view = this.views.find((v) => v.id === this.pageForm.value.viewId);
    ...
    updatePageFields(viewId: number | null): void {
        if (!viewId) {
            this.pageForm.patchValue({ tableFields: [], formFields: [] });
            return;
        }
        const view = this.views.find((v) => v.id === viewId);
        if (view) {
            this.availableFields = view.fields?.map((name) => ({ name, type: 'text', example: '' })) || this.availableFields;
            this.pageForm.patchValue({
                tableFields: view.fields || [],
                formFields: view.fields || [],
            });
        }
    }
}
